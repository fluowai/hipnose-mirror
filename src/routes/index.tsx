...
const ASSET_ORIGIN = "https://id-preview--6ae03129-e809-4e73-983e-875ca4ee10da.lovable.app";
const assetUrl = (asset: { url: string }) => `${ASSET_ORIGIN}${asset.url}`;
...
const events = [
  [assetUrl(eventTransformacao), "Congresso Transformação"], [assetUrl(eventParada), "Eu Acredito na Parada"],
  [assetUrl(eventEnem), "Aulão ENEM Colégio Anglo"], [assetUrl(eventBahia), "Congresso Internacional HIPNOBAHIA"],
  [assetUrl(eventBrasilia), "Congresso Internacional HIPNODAY"],
];

const media = [
  [assetUrl(mediaVanguarda), "Vanguarda Mix (TV Vanguarda)", "https://www.youtube.com/watch?v=oDLRyyCrKLI"],
  [assetUrl(mediaBand), "Falando Nisso (TV Band Vale)", "https://www.youtube.com/watch?v=HsrK2KHR7K0"],
  [assetUrl(mediaGlobo), "Vida Real (Rede Globo)", "https://www.youtube.com/watch?v=pkvLEEf41Q8"],
  [assetUrl(mediaSbt), "Programa PRA VOCÊ (SBT SJC)", "https://youtu.be/7VkDXVAJzks"],
  [assetUrl(mediaModo), "Modo Vale (SBT SJC)", "https://youtu.be/7VkDXVAJzks"],
  [assetUrl(mediaRecord), "Empresários de Sucesso (Record News)", "https://www.youtube.com/watch?v=mfK3FsVtDZk"],
];
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    ...
    { property: "og:image", content: assetUrl(hero) },
    { name: "twitter:card", content: "summary_large_image" },
    { name: "twitter:image", content: assetUrl(hero) },
  ]}),
  component: Index,
});
...
function Index() {
  return <main className="overflow-hidden bg-background text-foreground">
    <section id="inicio" className="relative flex min-h-[760px] items-center border-b border-border bg-cover bg-center md:min-h-screen" style={{ backgroundImage: `url(${assetUrl(hero)})` }}>
      <picture className="pointer-events-none absolute inset-0 md:hidden"><img src={assetUrl(heroMobile)} alt="Ton Lucas em cenário de hipnose" className="h-full w-full object-cover" /></picture>
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-10 pt-28 md:px-10">
        <div className="max-w-md">
          <img src={assetUrl(logo)} alt="Ton Lucas — hipnose e psicologia" className="mb-8 h-auto w-48" />
    ...
    <section id="beneficios" className="relative bg-cover bg-center py-24 md:py-32" style={{ backgroundImage: `linear-gradient(90deg, var(--background) 0%, color-mix(in oklab, var(--background) 91%, transparent) 55%, color-mix(in oklab, var(--background) 58%, transparent) 100%), url(${assetUrl(benefits)})` }}>
    ...
    <section className="relative bg-cover bg-center py-24 md:py-32" style={{ backgroundImage: `linear-gradient(90deg, color-mix(in oklab, var(--background) 35%, transparent), var(--background) 54%), url(${assetUrl(problems)})` }}>
      <picture className="pointer-events-none absolute inset-0 md:hidden"><img src={assetUrl(problemsMobile)} alt="Pessoa em processo de transformação" className="h-full w-full object-cover opacity-30" /></picture>
